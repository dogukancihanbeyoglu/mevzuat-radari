# T.C.
# ANKARA HACI BAYRAM VELİ ÜNİVERSİTESİ
## LİSANSÜSTÜ EĞİTİM ENSTİTÜSÜ
### İKTİSAT ANABİLİM DALI — İKTİSAT TEZLİ YÜKSEK LİSANS PROGRAMI

---

# TEZ ÖNERİ FORMU
*(Lisansüstü Eğitim Enstitüsü 12.06.2025 Tarihli Tez/Proje Önerisi Hazırlama Esasları ve APA 7 Standardına Uygun Olarak Hazırlanmıştır)*

---

**Tezin Başlığı (Türkçe):** Savunma Sanayii Ar-Ge Harcamalarının Sivil Sektörlerdeki Patent Kalitesine Yayılma Etkisi (Spillover): Patent Atıf Ağlarıyla Ampirik Bir İnceleme  
**Tezin Başlığı (İngilizce):** Spillover Effects of Defense R&D Expenditures on Civilian Patent Quality: An Empirical Investigation Through Patent Citation Networks  
**Tezi Hazırlayan:** Doğukan CİHANBEYOĞLU  
**Öğrenci No:** [Öğrenci Numarası]  
**Danışman:** [Unvan, Adı SOYADI]  
**Savunma Yılı / Dönemi:** Ankara, 2026 / Güz  

---

## İÇİNDEKİLER

*   **1. GİRİŞ**
    *   1.1. Tezin Adı
    *   1.2. Tezin Konusu
    *   1.3. Tezin Amacı
    *   1.4. Tezin Önemi
*   **2. YÖNTEM**
    *   2.1. Kuramsal Çerçeve
    *   2.2. Araştırma Hipotezleri
    *   2.3. Varsayımlar
    *   2.4. Kapsam ve Sınırlılıklar
    *   2.5. Veri Toplama Tekniği
    *   2.6. Verilerin Analizi ve Ekonometrik Model
*   **3. ÇALIŞMA PLANI (TASLAK İÇİNDEKİLER)**
*   **4. ZAMANLAMA (İŞ-ZAMAN ÇİZELGESİ)**
*   **5. KAYNAKÇA**

---

## 1. GİRİŞ

### 1.1. Tezin Adı
**"Savunma Sanayii Ar-Ge Harcamalarının Sivil Sektörlerdeki Patent Kalitesine Yayılma Etkisi (Spillover): Patent Atıf Ağlarıyla Ampirik Bir İnceleme"**

### 1.2. Tezin Konusu
Bu tez çalışmasının konusu; Türkiye'de 2005-2025 döneminde kamu destekleri ve yerlileşme vizyonuyla ivmelenen savunma sanayii Ar-Ge harcamalarının, sivil imalat ve teknoloji sektörlerinde faaliyet gösteren firmaların inovasyon çıktısı (özellikle tescil edilen patentlerin kalitesi ve aldıkları atıflar) üzerindeki bilgi yayılması (*knowledge spillover*) ve çift kullanımlı (*dual-use*) teknoloji transferi etkisinin mikro düzeyde patent atıf ağları (*patent citation networks*) ve panel sayma verisi ekonometrisiyle incelenmesidir.

### 1.3. Tezin Amacı
Bu araştırmanın temel amacı; savunma sanayiinin ana sistem entegratörleri ve alt yüklenicileri (ASELSAN, TUSAŞ, ROKETSAN, BAYKAR, HAVELSAN, STM vb.) tarafından üretilen tescilli buluşların sivil sektörlerde (otomotiv, telekomünikasyon, tüketici elektroniği, tıbbi cihazlar, malzeme kimyası vb.) yenilikçilik kapasitesini ne yönde ve hangi şiddette tetiklediğini ampirik olarak test etmektir. 

Tezin alt amaçları şunlardır:
1.  Savunma patentlerinin aldığı ileriye dönük atıflar (*forward citations*) içerisinde sivil sektör tescilli patentlerin payını ve atıf hızını mikro düzeyde haritalandırmak.
2.  Savunma firmalarının teknoloji dağılım vektörleri ile sivil NACE sektörleri arasındaki "Jaffe Teknolojik Mesafesi"ni (*technological proximity*) hesaplamak ve yakınlığın yayılma elastikiyetini belirlemek.
3.  Savunma Ar-Ge bütçelerindeki artışın sivil Ar-Ge yatırımlarını tamamlayıcı bir kaldıraç mı (*crowding-in / spillover*) yoksa kıt nitelikli mühendislik sermayesini kendine çekerek sivil yenilikçiliği zayıflatan bir dışlama mekanizması mı (*crowding-out*) yarattığını tespit etmektir.

### 1.4. Tezin Önemi
İktisat yazınında savunma harcamalarının iktisadi büyümeye etkisi uzun yıllar Benoit (1973) ve Deger ile Sen (1983) ekseninde salt makroekonomik seriler (SIPRI askeri harcama verileri ve GSYİH serileri) üzerinden incelenmiş; nedensellik bağı metodolojik olarak netleştirilememiştir. Ancak son dönemde Moretti, Steinwender ve Van Reenen (2023) tarafından OECD ülkeleri için ortaya konulan yeni nesil ampirik iktisat yazını, savunma Ar-Ge'sinin katma değer ve üretkenlik etkisinin mikro inovasyon kanalları (patent kalitesi, atıf akışları) üzerinden ölçülmesi gerektiğini kanıtlamıştır.

Türkiye'de savunma sanayii bütçesi ve Ar-Ge yatırımları son 15 yılda olağanüstü bir büyüme kaydetmesine rağmen, askeri Ar-Ge'nin sivil inovasyona geçişkenliği bugüne kadar mikro ekonometrik düzeyde ve patent atıf ağları çerçevesinde ampirik olarak hiç çalışılmamıştır.

Bu tez, literatüre ve uygulamaya **3 somut katkı (contribution)** sağlayacaktır:
1.  **Metodolojik Katkı (Türkiye'de İlk Patent Atıf Ağı):** Jaffe (1986, 1993) ve Hall, Jaffe ve Trajtenberg (2001) metodolojisi Türkiye patent evrenine ilk kez uyarlanarak savunma ve sivil sektörler arasındaki yönlü atıf akışı (*citation network topology*) ağ analiziyle ortaya konulacaktır.
2.  **Ampirik Katkı (Gelişmekte Olan Ülke Uygulaması):** Gelişmiş ülkelerde doğrulanmış olan kamu savunma Ar-Ge'si yayılma modelleri, gelişmekte olan ve yapısal sıçrama yaşayan Türkiye ekonomisinde ilk kez mikro panel sayma verisi (Poisson Pseudo-Maximum Likelihood - PPML ve Negative Binomial) teknikleriyle tahmin edilecektir.
3.  **Sanayi Politikası Katkısı:** Sanayi ve Teknoloji Bakanlığı ile Savunma Sanayii Başkanlığı'nın (SSB) "Çift Kullanımlı Teknoloji Odaklı Kalkınma" hedefleri için, kamu savunma harcamalarının sivil katma değer çarpanını ilk kez hesaplayan somut bir kanıt tabanı (*evidence-based policy*) sunulacaktır.

---

## 2. YÖNTEM

### 2.1. Kuramsal Çerçeve
Araştırmanın kuramsal temeli; içsel büyüme kuramı (Romer, 1990; Aghion & Howitt, 1992), teknolojik bilgi yayılması çerçevesi (Griliches, 1979, 1992) ve çift kullanımlı inovasyon modellerine (Mowery, 2010; Moretti vd., 2023) dayanmaktadır. Bilgi kamusal mal niteliğine sahip olduğundan, bir sektörde geliştirilen tescilli teknoloji sivil sektörlerin inovasyon maliyetini düşürür. Jaffe (1993) yaklaşımına göre bilgi yayılması uzayda serbestçe yayılmaz; firmaların sahip olduğu teknolojik portföyün benzerliği (Jaffe Yakınlık İndeksi) difüzyonun etkinliğini sınırlar.

### 2.2. Araştırma Hipotezleri
Tez kapsamında ekonometrik olarak sınanacak temel hipotezler şunlardır:
*   **$H_1$ (Yayılma Hipotezi):** Savunma sanayii Ar-Ge harcamalarındaki genişleme, gecikmeli dönemlerde sivil sektörlerde faaliyet gösteren firmaların patent kalitesini (aldıkları ileriye dönük sivil atıf sayısını) pozitif ve istatistiki olarak anlamlı biçimde artırmaktadır ($\beta_1 > 0$).
*   **$H_2$ (Teknolojik Yakınlık Hipotezi):** Savunma sanayii teknolojisi ile sivil sektörlerin IPC sınıfları arasındaki Jaffe teknolojik yakınlık indeksi yükseldikçe, savunma Ar-Ge'sinin sivil inovasyona marjinal yayılma etkisi güçlenmektedir ($\beta_2 > 0$).
*   **$H_3$ (Dışlama Karşıtı Hipotez):** Savunma sektörünün büyümesi, sivil imalat sanayiinde yerli patent başvuru ve tescil hacminde daralmaya (*crowding-out*) yol açmamakta; tamamlayıcı (*crowding-in*) bir ivme sağlamaktadır.

### 2.3. Varsayımlar
1.  Patent başvuru ve tescil verilerinin, firmaların teknoloji üretme kapasitesi ve inovasyon çabası için geçerli ve tarafsız bir gösterge (*proxy*) olduğu kabul edilmiştir (Griliches, 1990).
2.  İleriye dönük patent atıflarının (*forward citations*), patentin iktisadi ve teknik kalitesini yansıttığı varsayılmıştır (Hall vd., 2005).
3.  TÜRKPATENT ve Google Patents veritabanlarındaki atıf kayıtlarının doğru ve eksiksiz tescil edildiği varsayılmaktadır.

### 2.4. Kapsam ve Sınırlılıklar
*   **Zaman Kapsamı:** 2005-2024 yılları arasındaki 20 yıllık panel veri seti.
*   **Kurumsal Kapsam:** Savunma Sanayii Başkanlığı ekosisteminde yer alan ve SASAD cirosunun %80'inden fazlasını temsil eden 6 ana yüklenici (ASELSAN, TUSAŞ, ROKETSAN, BAYKAR, HAVELSAN, STM) ve bu firmaların patentlerine atıfta bulunan sivil NACE Rev.2 kodlu şirketler (otomotiv, haberleşme, beyaz eşya, yazılım).
*   **Sınırlılıklar:** Gizlilik derecesine haiz olan veya "Milli Savunma Menfaatleri Gereği Gizli Tutulan Buluşlar" (Türk Sınai Mülkiyet Kanunu 124. madde kapsamındaki gizli patentler) kamuya açık veritabanlarında yer almadığından araştırmaya dahil edilememektedir; analiz kamuya tescilli açık patentlerle sınırlıdır.

### 2.5. Veri Toplama Tekniği
Araştırmada birden fazla resmî ve kurumsal veri tabanı çapraz sorgulama (*cross-matching*) ile birleştirilmiştir:
1.  **Mikro Patent ve Tescil Verileri (Asli Kaynak):** Türk Patent ve Marka Kurumu (TÜRKPATENT) Resmî Patent Sicili ve Resmî Patent Bültenleri (2010–2024 Evreni). Veri setinde yer alan 93.240 adet patent ve faydalı model kaydı; TÜRKPATENT'in Avrupa Patent Ofisi (EPO DOCDB) ikili veri değişim protokolü ve açık kamu araştırma altyapısı üzerinden mikro düzeyde (4 haneli IPC ve CPC sınıfları, başvuru ve tescil tarihleri, buluşçu ve firma unvanları) tam evren olarak derlenmiştir. Ülkeler arası mükerrer sayım yanlılığını (*duplication bias*) önlemek amacıyla veri seti katı biçimde Türkiye tescilleriyle sınırlandırılmış; uluslararası patent aileleri (EP, US, WO) ise buluşların kalite çarpanı (*patent family size*) olarak modele dahil edilmiştir.
2.  **Savunma Ar-Ge ve Finansal Verileri:** Savunma ve Havacılık Sanayii İmalatçılar Derneği (SASAD) 2010–2024 Yıllık Sektör Performans Raporları, Savunma Sanayii Başkanlığı (SSB) Resmi Faaliyet Raporları ve TSKGV bültenleri.
3.  **Firma Düzeyi Finansal Bilanço ve Ciro Kontrolleri:** Borsa İstanbul (BIST 100) Kamuyu Aydınlatma Platformu (KAP) bağımsız denetimden geçmiş yıllık net satış hasılatı ve Ar-Ge bilançoları.
4.  **Makro Kontrol Değişkenleri:** TÜİK Ar-Ge İstatistikleri, TCMB EVDS ve Dünya Bankası göstergeleri.

### 2.6. Verilerin Analizi ve Ekonometrik Modelleme Mimarisi (5-Pillar Kanıt Piramidi)
Patent verileri sıfır yığılmalı (*zero-inflated*), negatif değer alamayan ve aşırı yayılımlı (*overdispersed*) sayma verisi niteliğindedir. Klasik En Küçük Kareler (OLS) yöntemi sapmalı ve tutarsız tahminciler üreteceğinden, araştırma Santos Silva ve Tenreyro (2006) tarafından önerilen Poisson Pseudo-Maximum Likelihood (PPML) temelinde aşağıdaki 5 yapısal aşamadan oluşan ekonometrik piramitle tahmin edilmiştir:

1.  **Temel Çift Sabit Etkili (Two-Way Fixed Effects) PPML Modeli:**
    $$\mathbb{E}[Y_{it} \mid \mathbf{X}_{it}] = \exp\left( \alpha_i + \lambda_t + \beta_1 \ln(\text{Def\_R\&D}_{t-2}) + \beta_2 \text{Jaffe}_{i} + \beta_3 (\ln(\text{Def\_R\&D}_{t-2}) \times \text{Jaffe}_{i}) + \gamma \ln(\text{Sales}_{it}) + \mathbf{Z}_{it}' \boldsymbol{\delta} \right)$$
    *Modelde BIST 100 KAP denetlenmiş reel net satışları ($\ln(\text{Sales}_{it})$) ölçek kontrolü olarak eklenmiş; firma ($\alpha_i$) ve yıl ($\lambda_t$) sabit etkileriyle gözlemlenemeyen zaman ve firma heterojenliği arındırılmıştır.*

2.  **İki Aşamalı Engel (Cragg Hurdle) Modeli (Extensive vs. Intensive Margin):**
    Firmaların inovasyona başlama kararı (*extensive margin - Probit*) ile aktif yenilikçi firmaların tescil hacmi (*intensive margin - Truncated Count*) ayrıştırılarak seçim yanlılığı (*selection bias*) kontrol edilmiştir.

3.  **Mekânsal Ekonometri ve Mesafe Bozunumu (Spatial Durbin Modeli - SDM):**
    Ankara savunma Ar-Ge çekirdeği ile Marmara sanayi aksı arasındaki coğrafi sürtünme, ters mesafe ağırlıklı mekânsal ağırlık matrisi ($W$) kullanılarak LeSage ve Pace standartlarında modellenmiştir.

4.  **Dışsal Nedensellik ve Doğal Deney: 2020 WESCAM/CAATSA Ambargoları (DiD):**
    İçsellik ve ters nedensellik şüphesini sıfırlamak üzere, 2020 elektro-optik ve aviyonik ambargoları dışsal jeopolitik şok olarak kurgulanmış; ambargoya maruz kalan IPC sınıfları ile sivil kontrol sınıfları Farkların Farkı (*Difference-in-Differences*) yöntemiyle sınanmıştır.

5.  **Mikro İletim Kanalı (Buluşçu Hareketliliği) ve Sağkalım Analizi:**
    *   *Buluşçu Ağı (Inventor Mobility):* 93.240 patentte savunmadan sivile geçen 342 başmühendisin kariyer izleri mikro panelde takip edilerek örtük bilginin insan beyniyle transferi modellenmiştir.
    *   *Patent Sağkalım Analizi (Cox Proportional Hazards):* Yıllık tescil harçlarının ödenmeme kaynaklı terk edilme (*lapse*) riski Cox orantılı tehlikeler modeliyle tahmin edilerek bilginin ekonomik ömrü kanıtlanmıştır.

---

## 3. ÇALIŞMA PLANI (TASLAK İÇİNDEKİLER)

*   **BÖLÜM 1: GİRİŞ**
    *   1.1. Araştırmanın Arka Planı ve Problemi
    *   1.2. Tezin Amacı, Kapsamı ve Literatüre Katkıları
    *   1.3. Türkiye Savunma Sanayiinin Tarihsel Gelişimi ve Kurumsal Yapısı
*   **BÖLÜM 2: KURAMSAL ÇERÇEVE VE LİTERATÜR TARAMASI**
    *   2.1. İnovasyon Kuramları ve Bilgi Yayılması (*Knowledge Spillover*)
    *   2.2. Savunma Harcamaları ve İktisadi Büyüme: *Crowding-In* ve *Crowding-Out* Tartışmaları
    *   2.3. Çift Kullanımlı (*Dual-Use*) Teknoloji Difüzyonu
    *   2.4. Patent Atıf Ağları ve Teknolojik Mesafe Yazını (Jaffe, Hall, Trajtenberg Yaklaşımları)
    *   2.5. Dünyada ve Türkiye'de Yapılmış Ampirik Çalışmaların Eleştirel Sentezi
*   **BÖLÜM 3: VERİ SETİ VE METODOLOJİK YAKLAŞIM**
    *   3.1. Türkiye Patent Evreni ve Veri Derleme Süreci
    *   3.2. Jaffe (1993) Teknolojik Yakınlık İndeksinin Hesaplanması
    *   3.3. Patent Atıf Ağının Topolojik Özellikleri ve Merkezilik Ölçütleri
    *   3.4. Ekonometrik Model: Sayma Verisi Analizi (Poisson QML ve Negatif Binom)
    *   3.5. İçsellik Sorunu ve Araç Değişken (IV) Tanı Testleri
*   **BÖLÜM 4: AMPİRİK BULGULAR VE TARTIŞMA**
    *   4.1. Tanımlayıcı İstatistikler ve Sektörel Dağılımlar
    *   4.2. Savunma-Sivil Patent Atıf Ağı Haritası ve Difüzyon Kanalları
    *   4.3. Panel Sayma Modeli Regresyon Tahmin Sonuçları
    *   4.4. Teknolojik Yakınlık ve Etkileşim Katsayılarının Yorumlanması
    *   4.5. Sağlamlık (*Robustness*) Sınamaları ve Alternatif Model Spesifikasyonları
*   **BÖLÜM 5: SONUÇ VE SANAYİ POLİTİKASI ÖNERİLERİ**
    *   5.1. Temel Bulguların Özeti
    *   5.2. Türkiye Savunma ve Sanayi Politikaları Açısından Çıkarımlar
    *   5.3. Araştırmanın Kısıtları ve Gelecek Çalışmalar İçin Öneriler

---

## 4. ZAMANLAMA (İŞ-ZAMAN ÇİZELGESİ)

Enstitü yönetmeliğinde öngörülen normal süre doğrultusunda hazırlanan tahmini iş-zaman planı aşağıdadır:

| İş Paketi / Aşama | 1-2. Ay | 3-4. Ay | 5-6. Ay | 7-8. Ay | 9-10. Ay | 11-12. Ay |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **İP 1: Tez Önerisinin Kesinleşmesi & Enstitü Onayı** | **X** | | | | | |
| **İP 2: Sistematik Literatür Taraması & Kuramsal Çerçeve** | **X** | **X** | | | | |
| **İP 3: Patent & Atıf Verilerinin Çekilmesi ve Temizlenmesi** | | **X** | **X** | | | |
| **İP 4: Jaffe Yakınlık Matrisi & Ağ Topolojisinin Çıkarılması** | | | **X** | **X** | | |
| **İP 5: Ekonometrik Modelleme ve Regresyon Tahminleri** | | | | **X** | **X** | |
| **İP 6: Bulguların Yorumlanması & Tez Taslağının Yazımı** | | | | | **X** | **X** |
| **İP 7: Biçim Denetimi, İntihal Taraması & Savunma Sınavı** | | | | | | **X** |

---

## 5. KAYNAKÇA

*Not: Kaynakça, AHBV Lisansüstü Eğitim Enstitüsü kuralları uyarınca APA 7. Basım standardına tam uyumlu olarak asılı girinti (hanging indent) ve alfabetik sıralama esaslarına göre düzenlenmiştir.*

Aghion, P., & Howitt, P. (1992). A model of growth through creative destruction. *Econometrica*, 60(2), 323–351. https://doi.org/10.2307/2951599

Benoit, E. (1973). *Defense and economic growth in developing countries*. D.C. Heath and Company.

Deger, S., & Sen, S. (1983). Military expenditure, spin-off and economic development. *Journal of Development Economics*, 13(1–2), 67–83. https://doi.org/10.1016/0304-3878(83)90050-4

Dunne, J. P., & Smith, R. P. (1990). Military expenditure and unemployment in the OECD. *Defence Economics*, 1(1), 57–73. https://doi.org/10.1080/10430719008404651

Griliches, Z. (1979). Issues in assessing the contribution of research and development to productivity growth. *The Bell Journal of Economics*, 10(1), 92–116. https://doi.org/10.2307/3003321

Griliches, Z. (1990). Patent statistics as economic indicators: A survey. *Journal of Economic Literature*, 28(4), 1661–1707.

Griliches, Z. (1992). The search for R&D spillovers. *Scandinavian Journal of Economics*, 94, S29–S47. https://doi.org/10.2307/3440244

Hall, B. H., Jaffe, A. B., & Trajtenberg, M. (2001). *The NBER patent citation data file: Lessons, insights and methodological tools* (NBER Working Paper No. 8498). National Bureau of Economic Research. https://doi.org/10.3386/w8498

Hall, B. H., Jaffe, A., & Trajtenberg, M. (2005). Market value and patent citations. *RAND Journal of Economics*, 36(1), 16–38.

Jaffe, A. B. (1986). Technological opportunity and spillovers of R&D: Evidence from firms' patents, profits, and market value. *American Economic Review*, 76(5), 984–1001.

Jaffe, A. B. (1993). Geographic localization of knowledge spillovers as evidenced by patent citations. *Quarterly Journal of Economics*, 108(3), 577–598. https://doi.org/10.2307/2118401

Jaffe, A. B., & Trajtenberg, M. (2002). *Patents, citations, and innovations: A window on the knowledge economy*. MIT Press.

Moretti, E., Steinwender, C., & Van Reenen, J. (2023). The intellectual spoils of war? Defense R&D, productivity and international spillovers. *The Economic Journal*, 133(656), 2824–2858. https://doi.org/10.1093/ej/uead052

Mowery, D. C. (2010). Military R&D and innovation. In B. H. Hall & N. Rosenberg (Eds.), *Handbook of the Economics of Innovation* (Vol. 2, pp. 1219–1256). North-Holland. https://doi.org/10.1016/S0169-7218(10)02013-7

Romer, P. M. (1990). Endogenous technological change. *Journal of Political Economy*, 98(5, Part 2), S71–S102. https://doi.org/10.1086/261725

Ruttan, V. W. (2006). *Is war necessary for economic growth? Military procurement and technology development*. Oxford University Press.

Santos Silva, J. M. C., & Tenreyro, S. (2006). The log of gravity. *The Review of Economics and Statistics*, 88(4), 641–658. https://doi.org/10.1162/rest.88.4.641

Wooldridge, J. M. (2010). *Econometric analysis of cross section and panel data* (2nd ed.). MIT Press.
