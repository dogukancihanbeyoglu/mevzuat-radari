...
with full UTF-8 / Turkish character support (ç, ğ, ı, ö, ş, ü, İ, Ğ, Ş, Ç, Ö, Ü).
"""
import os
...
def generate_pdf_report(report: DailyAuditReport, output_path: str) -> str:
    ...
    # Refined, executive typography styles
    ...
    story.append(
        Paragraph(
            f"<b>Kurum / Şirket:</b> {report.company_name} | <b>Tarih:</b> {report.date} | <b>Sayı:</b> {report.gazette_number or 'Günlük Sayı'}<br/>"
            f"<b>Taranan Madde Sayısı:</b> {report.total_scanned} | <b>İlgili Bulunan Madde:</b> {report.relevant_count} | <i>Oluşturulma: {report.generated_at}</i>",
            subtitle_style,
        )
    )
    ...
    if not report.evaluations:
        no_item_text = (
            "<b>Bu Tarih İçin Şirketi İlgilendiren Kritik Karar Bulunmadı:</b><br/>"
        ...
        for idx, ev in enumerate(report.evaluations, 1):
            ...
            meta_data = [
                ...
                [
                    Paragraph(f"<b>Düzenleyen Kurum:</b> {ev.item.institution or 'Resmî Gazete'}", body_style),
            ...
            item_elements.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#e2e8f0"), spaceAfter=8))
